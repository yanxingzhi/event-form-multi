...
export async function onRequestPost({ request, env }) {
    try {
      ...
      // 5. 执行删除操作（注意：表格第一行是表头，数据从 index 1 开始）
            ...
            requests: [
              {
                deleteDimension: {
                  range: {
                    sheetId: 0, // 默认第一个工作表是 0，如不是请替换为实际 ID
                    dimension: "ROWS",
                    startIndex: rowIndex, // 包含该行
                    endIndex: rowIndex + 1, // 不包含该行
                  },
                },
              },
            ],
      ...
      await sendToLine({userId, text: `｢${eventsJson.find(e => e.id == a).title}, LINE_CHANNEL_ACCESS_TOKEN: env.LINE_CHANNEL_TOKEN」：お申し込みはキャンセルされました、、ありがとうございました`})
